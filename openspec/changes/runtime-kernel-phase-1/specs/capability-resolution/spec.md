...
#### Scenario: Capability dispatch admission is fenced at one synchronous linearization point
- **WHEN** Main loses authority before the post-routing authority check and exact-provider request reservation/send complete
- **THEN** no provider command is created, while an invocation that crossed that synchronous admission point before authority loss remains admitted and may settle
