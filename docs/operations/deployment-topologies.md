...
The local control endpoint is trusted. Path-based installation on a follower
may admit content-addressed immutable artifact bytes before the semantic
installation crosses the leadership fence. The operation returns
`COORDINATION_NOT_LEADER`, leaving installations and deployments semantic state
unchanged. An authorized local client can still consume artifact storage this
way. Operators must treat repeated follower ingress as a storage denial of
service risk even though fencing protects semantic state.
